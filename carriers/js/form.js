...
var vm = new Vue({
	...
	methods: {
		...
		submitFun: function(){ // 提交
			...
			request.PostInfo_new(request.tms_group_addgroup,data,function(res){
		  		if (self.from){
		  			plus.webview.getWebviewById(self.from).evalJS("set_carriers()");
		  			// plus.webview.currentWebview().opener().evalJS("set_customer()");
		  		} else {
		  			plus.webview.getWebviewById('../carriers/list.html').reload();
		  		}
		  		mui.back();
		  	},function(res){
