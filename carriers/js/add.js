		...
		submitFun: function(){ // 提交
			...
			if(type == 1){ // 执行添加动作
				...
				if (project_type == 'TMS3PL') {
					request.PostInfo_new(request.tms_address_addAddress,submitData,function(){
						plus.webview.currentWebview().opener().evalJS("refreshData()");
						mui.back();
					},function(){})						
				}else{				
					request.PostInfo_new(request.api_address_addAddress,submitData,function(){
						plus.webview.currentWebview().opener().evalJS("refreshData()");
						mui.back();
					},function(){})						
				}
				...
				if (project_type == 'TMS3PL') {
				account.updateAddress(submitData,function(){
					// console.log(111111);
					plus.webview.currentWebview().opener().evalJS("refreshData()");
					mui.back();
				})
			}else{
				account.updateAddress_api(submitData,function(){
					// console.log(111111);
					plus.webview.currentWebview().opener().evalJS("refreshData()");
					mui.back();
				})
