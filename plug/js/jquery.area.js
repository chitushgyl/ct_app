...
/*定义三级省市区数据*/
var province = ["北京市", "上海市", "广东省", "浙江省", "江苏省","天津市", "湖北省","湖南省","重庆市","四川省","河北省", "河南省", "山西省", "山东省", "内蒙古自治区", "辽宁省", "吉林省", "黑龙江省", 
	"福建省", "江西省", "安徽省",  "广西壮族自治区", "海南省",  "贵州省", "云南省", "西藏自治区", "陕西省", "甘肃省", "青海省",
...
function selectP(p, flag) {
	...
	if (flag == 0) {
		...
		// $('.area-list li').expressArea.addClass("current");
	}
...
function selectC(p, c) {
	...
	if (sCity != "省直辖县级行政单位") {
		if (sCity == "东莞市" || sCity == "中山市" || sCity == "儋州市" || sCity == "嘉峪关市") {
			...
			clockArea();
		} else if (sCity == "市辖区" || sCity == "市辖县" || sCity == "香港岛" || sCity == "九龙半岛" || sCity == "新界" || sCity == "澳门半岛" ||
			sCity == "离岛" || sCity == "无堂区划分区域") {
			expressArea += "";
		} else {
			expressArea += sCity + "  ";
		}
	...
	if (sCity == "") {
		...
		console.log(sCity);                              // 获得选中的市
		$('.cityone').html(sCity).addClass("current");
		// $('.cityone').html = sCity;
		$(".cityone").html(sCity);
		// document.getElementById("cityone").innerHTML = sCity;
