...
var vm = new Vue({
    ...
    mounted: function() {
		...
		mui.plusReady(function(){
			...
			self.status_page=page.type;
			self.nav_list.forEach((item,index)=>{
				if(item.type == self.status_page){
	...
	methods:{
		...
		to_detail:function(index){
			...
			mui.openWindow({
			    ...
			    id:"order_user/myOrderDetail.html",
			    extras:{
			      	self_id: conid,
			      	type:1
			    },
		...
		vehical_done: function(index,id){//操作按钮
			var self = this;
			...
			if(id == 120){
				mui.confirm('订单取消后将无法恢复', '确定取消订单？', btnArray, function(e) {
					if (e.index == 1) {
						...
						request.PostInfo_new(request.api_take_order_cancel,data,function(res){
							mui.toast(res.msg);
			...
			if(id == 121){
				...
				if(id == 122){
					mui.confirm('确认该订单已经完成吗？', '完成订单', btnArray, function(e) {
						if (e.index == 1) {
							...
							request.PostInfo_new(request.api_take_carriage_done,data,function(res){
								mui.toast(res.msg);
								vm.mescroll.resetUpScroll();
							},function(res){});
						}
				...
				if(id == 123){
					mui.openWindow({
					    url:'../order_user/uploadImg.html',
					    id:'../order_user/uploadImg.html',
					    styles:{},
					    extras:{
					      data:data
					    },
