...
var vm = new Vue({
    ...
    data: {
        ...
        mescroll: null,
		nav_list:[
			{name:'待调度',type:'status2'},
			{name:'运输中',type:'status3'},
			{name:'已完成',type:'status4'},
			{name:'已取消',type:'status5'},
		],
		current:0,
		status_type:'',
		list: [],
    ...
    mounted: function() {
		...
		mui.plusReady(function(){
			...
			self.status_type=page.type;
			self.nav_list.forEach((item,index)=>{
				if(item.type == self.status_type){
					self.sliderNum=index+1;
	...
	methods:{
		...
		to_detail:function(index){
			...
			mui.openWindow({
			    ...
			    id:"3pl_customer/orderDetail.html",
			    extras:{
			      	self_id: conid,
			      	type:1
			    },
		...
		vehical_done: function(index){//内部订单 完成
			var self = this;
			var uoid = self.list[index].id;
			...
			mui.confirm('确认该订单已经送达吗？', '订单送达', btnArray, function(e) {
				if (e.index == 1) {
					...
					request.PostInfo_new(request.app_driver_vehical_done,data,function(res){
						mui.toast(res.msg);
						vm.mescroll.resetUpScroll();
