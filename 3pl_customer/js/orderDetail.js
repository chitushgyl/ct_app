	...
	methods: {
		getDetail: function(){
			...
			request.PostInfo_new(request.api_order_details,data,function(res){
				...
				// self.startstr = JSON.parse(data.line_start_contant);
				// self.endstr = JSON.parse(data.line_end_contant);
				// self.receipt = data.receipt ? JSON.parse(data.receipt) : '';
				// self.driverinfo = data.driverinfo ? JSON.parse(data.driverinfo) : '';
				if (self.receipt) {
