...
var vm = new Vue({
	...
	methods:{
		...
		getTransportData:function(pageNum){
			...
			console.log("3pl线路"+JSON.stringify(data));
			request.PostInfo_new(request.tms_line_linePage,data,function(response){
				console.log("3pl线路:"+JSON.stringify(response));
		...
		deleteLine: function(index){ // 删除线路
			var self  = this;
			mui.confirm('删除后数据将无法恢复','确定删除线路？',["确认","取消"],function(e){
				if(e.index == 0){
					var self_id = self.lineList[index].self_id;
					var data = {
					...
					if(project_type=='TMS3PL'){
						...
						request.PostInfo_new(request.tms_line_lineDelFlag,data,function(response){
							self.lineList.splice(index, 1);
							mui.toast('删除成功');
		...
		createLine: function(index){ // 编辑线路
			var self  = this;
			var self_id = self.lineList[index].self_id;
			console.log("self_id："+self_id);
			mui.openWindow({
			    url:"create_line.html",
			    id:"line/create_line.html",
			    extras:{
			    	self_id:self_id
			    },
			    waiting:{
