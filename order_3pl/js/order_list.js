...
var vm = new Vue({
    ...
    mounted: function() {
		...
		mui.plusReady(function(){
			...
			console.log("page:"+JSON.stringify(page.id));
	...
	methods:{
		...
		getListDataFromNet:function(pageNum,status){
			...
			request.PostInfo_new(request.tms_order_orderPage,data,function(res){
			...
			if(id == 124){
				mui.confirm('订单取消后将无法恢复', '确定取消订单？', btnArray, function(e) {
					if (e.index == 1) {
						...
						request.PostInfo_new(request.tms_order_orderCancel,data,function(res){
							mui.toast(res.msg);
							vm.mescroll.resetUpScroll();
							// localStorage.is_now_action = 1;
							// plus.webview.getWebviewById('person/index-menu.html').reload();
